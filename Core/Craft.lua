...
function Craft:ScanTradeSkill()
    ...
    for i = 1, n do
        ...
        if skillName and skillType ~= "header" then
            ...
            -- enchanting scrolls report the enchant, not an item, on some
            -- recipes; without an item id there is nothing to price
            if id then
...
function Craft:Evaluate(inputID)
    ...
    for _, o in pairs(rec.outputs) do
        ...
        rows[#rows+1] = {
            id = o.id, name = o.name or "?", perOp = perOp, total = o.total,
...
function Craft:EvaluateRoute(craftedID, rec)
    ...
    for _, g in ipairs(rec.reagents) do
        ...
        rows[#rows+1] = {
            -- uncached reagents have no name yet; asking for the info is what
            -- makes the client fetch it, so the next refresh shows it
            id = g.id, name = g.name or (U:ItemInfo(g.id) or {}).name or ("item #"..g.id),
...
function Craft:AllConversions()
    ...
    for _, rec in ipairs(self:All()) do
        ...
        local info = U:ItemInfo(rec.id)
        out[#out+1] = {
            id = rec.id, kind = "learned",
            name = rec.name or (info and info.name) or "?",
...
-- a newly met scroll changes the index, so let it be rebuilt
function Craft:InvalidateUsedIndex() usedIndex = nil end
...
function Craft:UsedIn(reagentID)
    ...
    for _, craftedID in ipairs(usedIndex[reagentID] or {}) do
        ...
        if rp then
            ...
            out[#out+1] = {
                id         = craftedID,
                name       = rp.name or (U:ItemInfo(craftedID) or {}).name or ("item #"..craftedID),
